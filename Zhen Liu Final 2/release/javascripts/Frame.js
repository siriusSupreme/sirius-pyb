  ...
  function searchHandler( e ) {
    if ( searchDom.classList.contains( 'shown' ) ) {
      ...
      if ( value ) {
        ...
        if ( value === 'Chinese cusine' ) _List.createList( self.x, self.y, self.width, self.height ).draw( svg, radius, attr, true )
        else _List.createList( self.x, self.y, self.width, self.height ).draw( svg, radius, attr, false )
      }
    } else {
      // 隐藏标题
